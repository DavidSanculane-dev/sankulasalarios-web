...
import { deleteSystemUserAction, updateSystemUserAction } from "./actions";
...
export default function UserActions({ user }: UserActionsProps) {
  ...
  const handleDelete = () => {
    if (confirm(`Tem a certeza que deseja revogar o acesso do e-mail ${user.email}?`)) {
      startTransition(async () => {
        const res = await deleteSystemUserAction(user.id);
  ...
  const handleEditSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const res = await updateSystemUserAction(user.id, formData);
      if (res?.error) {
        setError(res.error);
      } else {
        setIsEditOpen(false);
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden border border-slate-200 text-left">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-900">Editar Permissões do Utilizador</h3>
            ...
            <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase">E-mail Corporativo</label>
                <input disabled type="text" value={user.email || "—"} className="mt-1 block w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-500 cursor-not-allowed focus:outline-none" />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase">Nível de Permissão (Role) *</label>
                <select name="role" defaultValue={user.role} className="mt-1 block w-full px-3 py-2 border border-slate-300 bg-white rounded-lg text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase">Alterar Palavra-passe (Opcional)</label>
                <input type="password" name="password" placeholder="Deixe em branco para manter a atual" className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
